...
const NewsDetail_02 = ({ params, draftKey }: NewsDetailProps) => {
  const { id } = params
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      try {
        ...
        const currentPost = await newsFetch.get(id, draftKey)
        if (!mounted) return
      ...
      <div className="max-w-[1200px] mx-auto mt-10 lg:mt-24 px-3">
        ...
        {relatedPosts.length > 0 && (
          <section className="mt-10 lg:mt-24 bg-white p-5 lg:p-16">
            <h2 className="text-2xl mb-6 text-accentColor">関連ニュース</h2>
